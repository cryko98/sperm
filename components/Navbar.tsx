...
export const Navbar: React.FC = () => {
  ...
  return (
    <nav className="flex justify-between items-center py-5 sticky top-4 z-40 bg-black/90 backdrop-blur-xl rounded-full px-8 border border-white/10 mx-2 shadow-2xl group">
      ...
      <div className="flex gap-6 sm:gap-10 items-center text-[11px] font-bold uppercase tracking-[0.2em]">
        <button onClick={() => scrollTo('tokenomics')} className="hover:text-white text-gray-400 transition-colors">Stats</button>
        <button onClick={() => scrollTo('chart')} className="hover:text-white text-gray-400 transition-colors">Chart</button>
        <a 
            href="https://pump.fun/coin/8a9vJCNjiM7LW43JeAYjf4FDVzi2qhoEF4fCWqwgpump" 
            target="_blank" 
            rel="noopener noreferrer"
            className="bg-white text-black px-7 py-2.5 rounded-full hover:bg-gray-200 transition-all transform active:scale-95 shadow-[0_0_10px_rgba(255,255,255,0.2)]"
        >
          BUY $SPERM
        </a>
      </div>
      
      <div className="absolute -bottom-1 left-1/2 -translate-x-1/2 w-1/3 h-[2px] bg-white/20 opacity-0 group-hover:opacity-100 transition-all duration-500"></div>
    </nav>
  );
};